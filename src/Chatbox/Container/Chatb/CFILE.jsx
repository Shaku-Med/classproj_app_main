...
let CFILE = ({ val, filtME }) => {
    const { setPV, pvT, setpvT, users, owner, chid } = useContext(Conn);
            ...
            <div className={`fileuploadpathssd ${val.file.length < 1 ? ' pointer-events-none' : `p-2`} transition-all flex overflow-x-auto gap-1 overflow-y-hidden w-full`}>
                {
                    (filtME(val.file, val.time, true) || []).map((v, key) => {
                        if (typeof v === 'object') {
                            return (
                                <div onClick={e => {
                                    setPV(v.hasOwnProperty('tty') ? v.file : v, v.hasOwnProperty('isr'), v.tty, v.type, v.id, v.hasOwnProperty('fileLength') ? v.fileLength : null)
                                }} key={key} className="flexaidna transition-all cursor-pointer hover:brightness-125 max-h-[70px] overflow-hidden flex items-center relative justify-center flex-col max-w-[70px] min-w-[70px] shadow-md p-1 brd rounded-md">
                                    {
                                        v.type.includes('image') ?
                                            <div className="aidnkkkaidnkdeaser892 w-full h-full">
                                                <Img len={v.hasOwnProperty('fileLength') ? v.fileLength : null} id={v.id} type={v.tty} isDEF={v.hasOwnProperty('isr')} loading='lazy' src={v.hasOwnProperty('tty') ? v.file : v} className={` w-full object-cover object-top h-full rounded-md brd shadow-md overflow-hidden`} />
                                            </div> :
